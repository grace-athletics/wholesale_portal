            ...
            <div className="space-y-2">
              ...
              <div className="relative rounded-md border bg-muted/50 p-3 pr-10">
                <code className="text-xs break-all text-muted-foreground select-all leading-relaxed">
                  {`javascript:(async()=>{let c=[...document.querySelectorAll('canvas')].filter(c=>c.offsetWidth>100&&c.offsetHeight>100).sort((a,b)=>b.width*b.height-a.width*a.height)[0];if(!c)return alert('No glove canvas found.');let n=4,frac=.18,wait=800,imgs=[];let r=c.getBoundingClientRect(),sx=r.left+r.width*.7,sy=r.top+r.height*.5,dx=-r.width*frac;const sleep=ms=>new Promise(r=>setTimeout(r,ms));const fire=(t,x,y)=>{let o={bubbles:true,clientX:x,clientY:y,buttons:1,button:0,pointerId:1,pointerType:'mouse'};try{c.dispatchEvent(new PointerEvent(t,o))}catch(e){c.dispatchEvent(new MouseEvent(t,o))}};for(let i=0;i<n;i++){imgs.push(c.toDataURL('image/png'));fire('pointerover',sx,sy);fire('pointerenter',sx,sy);fire('pointermove',sx,sy);fire('pointerdown',sx,sy);for(let s=1;s<=24;s++){fire('pointermove',sx+dx*s/24,sy)}fire('pointerup',sx+dx,sy);await sleep(wait)}let out=document.createElement('canvas'),w=c.width,h=c.height;out.width=w*2;out.height=h*2;let ctx=out.getContext('2d');ctx.fillStyle='black';ctx.fillRect(0,0,out.width,out.height);for(let i=0;i<imgs.length;i++){let im=new Image();im.src=imgs[i];await new Promise(res=>im.onload=res);ctx.drawImage(im,(i%2)*w,Math.floor(i/2)*h,w,h)}let a=document.createElement('a');let product=new URLSearchParams(location.search).get('product')||'glove';a.href=out.toDataURL('image/png');a.download=\`\${product}-glove-4-views.png\`;a.click();alert('Done — saved 1 image with all 4 views!')})();`}
                </code>
                <button
                  type="button"
                  onClick={() => {
                    navigator.clipboard.writeText(`javascript:(async()=>{let c=[...document.querySelectorAll('canvas')].filter(c=>c.offsetWidth>100&&c.offsetHeight>100).sort((a,b)=>b.width*b.height-a.width*a.height)[0];if(!c)return alert('No glove canvas found.');let n=4,frac=.18,wait=800,imgs=[];let r=c.getBoundingClientRect(),sx=r.left+r.width*.7,sy=r.top+r.height*.5,dx=-r.width*frac;const sleep=ms=>new Promise(r=>setTimeout(r,ms));const fire=(t,x,y)=>{let o={bubbles:true,clientX:x,clientY:y,buttons:1,button:0,pointerId:1,pointerType:'mouse'};try{c.dispatchEvent(new PointerEvent(t,o))}catch(e){c.dispatchEvent(new MouseEvent(t,o))}};for(let i=0;i<n;i++){imgs.push(c.toDataURL('image/png'));fire('pointerover',sx,sy);fire('pointerenter',sx,sy);fire('pointermove',sx,sy);fire('pointerdown',sx,sy);for(let s=1;s<=24;s++){fire('pointermove',sx+dx*s/24,sy)}fire('pointerup',sx+dx,sy);await sleep(wait)}let out=document.createElement('canvas'),w=c.width,h=c.height;out.width=w*2;out.height=h*2;let ctx=out.getContext('2d');ctx.fillStyle='black';ctx.fillRect(0,0,out.width,out.height);for(let i=0;i<imgs.length;i++){let im=new Image();im.src=imgs[i];await new Promise(res=>im.onload=res);ctx.drawImage(im,(i%2)*w,Math.floor(i/2)*h,w,h)}let a=document.createElement('a');let product=new URLSearchParams(location.search).get('product')||'glove';a.href=out.toDataURL('image/png');a.download=\`\${product}-glove-4-views.png\`;a.click();alert('Done — saved 1 image with all 4 views!')})();`);
                    setCodeCopied(true);
